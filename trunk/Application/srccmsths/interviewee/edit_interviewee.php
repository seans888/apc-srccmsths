...
if(isset($_GET['interview_no']) && isset($_GET['Exam_exam_no']) && isset($_GET['Exam_Applicant_applicant_no']))
{
    ...
    if($result = $dbh_interviewee->make_query())
    {
        ...
        extract($data);
        $orig_Exam_exam_no = $Exam_exam_no;
        $orig_Exam_Applicant_applicant_no = $Exam_Applicant_applicant_no;
...
elseif(xsrf_guard())
{
    ...
    if($_POST['submit'])
    {
        ...
        if($dbh_interviewee->check_uniqueness_for_editing($_POST)) ;
        else $message = "Record already exists with the same primary identifiers!";

        if($message=="")
        {
            $dbh_interviewee->edit($_POST);
...
$html->draw_header('Edit interviewee', $message, $message_type);
...
echo "<input type=hidden name='orig_Exam_exam_no' value='$orig_Exam_exam_no'>";
echo "<input type=hidden name='orig_Exam_Applicant_applicant_no' value='$orig_Exam_Applicant_applicant_no'>";

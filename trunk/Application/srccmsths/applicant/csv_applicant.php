<?php
//******************************************************************
//This file was generated by Cobalt, a rapid application development 
//framework developed by JV Roig (jvroig@jvroig.com).
//
//Cobalt on the web: http://cobalt.jvroig.com
//******************************************************************
require_once 'path.php';
init_cobalt('View applicant');
if(!isset($_POST['form_key'])) log_action("Module Access", $_SERVER['PHP_SELF']);

if(isset($_GET['filter_field_used']) && isset($_GET['filter_used']) && isset($_GET['page_from']))
{
    $page_from = htmlentities($_GET['page_from']);
    $filter_used = htmlentities($_GET['filter_used']);
    $filter_field_used = htmlentities($_GET['filter_field_used']);
}

if(xsrf_guard())
{
    extract($_POST);
    if($_POST['cancel']) 
    {
        log_action('Pressed cancel button', $_SERVER[PHP_SELF]);
        header("location: listview_applicant.php?filter_field=$filter_field_used&filter=$filter_used&page_from=$page_from");
    }

    if($_POST['submit'])
    {
        log_action('Pressed submit button', $_SERVER[PHP_SELF]);

        require_once 'validation_class.php';
        require_once 'subclasses/applicant.php';
        $dbh_applicant = new applicant;
        $validator = new validation;

        extract($_POST);

        if($message=="")
        {
            log_action("Exported table data to CSV", $_SERVER['PHP_SELF']);
            $timestamp = date('Y-m-d');
            $csv_name = '/' . $_SESSION['user'] . '_applicant_' . $timestamp . '.csv';
            $filename = TMP_DIRECTORY . $csv_name;
            $linkname = TMP_HYPERLINK_PATH . $csv_name;

            $csv_contents = $dbh_applicant->export_to_csv();

            $csv_file=fopen($filename,"wb");
            fwrite($csv_file, $csv_contents);
            fclose($csv_file);
            chmod($filename, 0755);

            $message='CSV file successfully generated: <a href="' . $linkname . '">Download the CSV file.</a>';
            $message_type='system';
        }
    }
}
require_once 'subclasses/applicant_html.php';
$html = new applicant_html;
$html->draw_header('CSV Exporter: applicant', $message, $message_type);
$html->draw_listview_referrer_info($filter_field_used, $filter_used, $page_from);

echo '<div class="container_mid">';
$html->draw_fieldset_header('CSV Filter Settings');
$html->draw_fieldset_body_start();

$html->draw_text_field('Learners No', 'learners_no', FALSE, 'text', TRUE);
$html->draw_text_field('First Name', 'first_name', FALSE, 'text', TRUE);
$html->draw_text_field('Last Name', 'last_name', FALSE, 'text', TRUE);
$html->draw_text_field('Middle Name', 'middle_name', FALSE, 'text', TRUE);
$html->draw_text_field('Date Of Birth', 'date_of_birth', FALSE, 'text', TRUE);
$html->draw_text_field('Gender', 'gender', FALSE, 'text', TRUE);
$html->draw_text_field('Year', 'year', FALSE, 'text', TRUE);


$html->draw_fieldset_body_end();
$html->draw_fieldset_footer_start();
$html->draw_submit_cancel();
$html->draw_fieldset_footer_end();
echo '</div>';

$html->draw_footer();
...
if(xsrf_guard())
{
    ...
    if($_POST['submit'])
    {
        ...
        if($message=="")
        {
            ...
            $csv_name = '/' . $_SESSION['user'] . '_section_' . $timestamp . '.csv';
            $filename = TMP_DIRECTORY . $csv_name;
            $linkname = TMP_HYPERLINK_PATH . $csv_name;

            $csv_contents = $dbh_section->export_to_csv();
...
$html->draw_header('CSV Exporter: section', $message, $message_type);
...
$html->draw_text_field('Teacher Teacher No', 'Teacher_teacher_no', FALSE, 'text', TRUE);
$html->draw_text_field('Teacher Department Dept No', 'Teacher_Department_dept_no', FALSE, 'text', TRUE);
